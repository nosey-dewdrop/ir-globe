/* dinamik bülten — kendini gerçek veriden yazar, uydurma yok.
   kaynak: data/news/*.json (google news, news.yml ile 6 saatte bir tazelenir) +
   data/layers/*.json (SIPRI yüzdeleri + bağ notları). her açılışta o anın en canlı
   bağını seçer, kaynaklı giriş + en yeni gerçek manşetleri dizer. */
(function () {
  ...
  Store.layerIndex().then(function (index) {
    ...
    return Promise.all([
      Promise.all(keys.map(function (k) { return Store.layer(k); })),
      Promise.all(keys.map(function (k) { return Store.news(k); })),
    ]).then(function (r) { render(keys, r[0], r[1]); });
  }).catch(function () {
    var el = document.getElementById("lead");
    if (el) el.innerHTML = "<p class=\"lead-body\">Henüz haber akışı yüklenmedi. Birazdan tekrar dene.</p>";
  });

  function render(keys, layers, newsBags) {
    // silah yüzdeleri + dünya payları + non-silah bağ notları (data/layers/*.json)
    var exp = {}, imp = {}, supShare = {}, recShare = {}, noteOf = {};
    layers.forEach(function (lay) {
      if (lay.shares) { supShare = lay.shares.sup || {}; recShare = lay.shares.rec || {}; }
      (lay.ties || []).forEach(function (t) {
        var edge = t.s + "→" + t.r;
        if (t.exp != null) exp[edge] = t.exp;
        if (t.imp != null) imp[edge] = t.imp;
        if (t.note) noteOf[lay.key + "|" + edge] = t.note;
      });
    });
    ...
    var all = [];
    var edgeStat = {};
    keys.forEach(function (layer, i) {
      var edges = newsBags[i] || {};
