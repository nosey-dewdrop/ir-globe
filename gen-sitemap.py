#!/usr/bin/env python3
"""Auto-generate sitemap.xml from every real .html page under the web root.

Run on every deploy so when Damla adds a country / relation / topic page the
sitemap updates itself and Google picks it up. No manual per-page indexing.

Rules:
- one <url> per .html file (index.html -> the directory URL, no filename)
- priority by section, changefreq by how often it moves
- lastmod = file mtime
- skips non-indexable pages (404, redirect stubs, noindex, admin, vendor)
"""
import os, re, datetime

WEB = os.path.dirname(os.path.abspath(__file__))
DOMAIN = "https://ir-globe.noseydewdrop.com"
...
SKIP_FILES = {"404.html"}
SKIP_DIRS = {"assets", "css", "js", "styles", "vendor", "data", "pdf",
             ".git", ".vercel", ".claude", ".operator", "node_modules",
             "admin", "docs", "scripts"}
